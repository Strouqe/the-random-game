...
export class MissionResultComponent implements OnInit {
	...
	constructor(
		...
		this.missionService.getResult(this.data.mission, this.data.party).then((results) => {
	...
	ngOnInit() {
		...
		this.dialogRef.afterClosed().subscribe(() => {
			// check if the user has compleated 7 missions
			if (this.userService.getUser().missionsCompleated.length >= 7) {
	...
	openMissionDialog(enterAnimationDuration: string, exitAnimationDuration: string): void {
